...
C COMPUTES THE SCALAR STRUCTURE
C U_BAR(L1,P1)(SLASH(Q)+RMB)U(L2,P2)
...
C CHECK ON CORRECT LABEL INPUT
      IP1=IDINT(P1(4))
      IQ=IDINT(Q(4))
      IP2=IDINT(P2(4))
     ...
     .             ' L2=',L2,' IP2=',IP2
        STOP
      ENDIF
C CHECK WHETHER THIS ONE HAS BEEN CALCULATED ALREADY
      IF(DONF(L1,IP1,IQ,L2,IP2).EQ.0) THEN
C THIS ONE NOT DONE YET: DO IT AND STORE THE RESULT IN ARRAY 'ANSF'
        IF(L1.EQ.L2) THEN
          A=2D0*RKDOT(Q,P2)
