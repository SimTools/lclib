...
C G(K1) + G(K2) ---> Z(QV) + B(P1) + B_BAR(P2)
C                     |
C                     +---> L(LEP1) + LEP_BAR(LEP2)
C THE B QUARKS HAVE TO BE ON-SHELL, THE LEPTONS MASSLESS
C THE OPTION IMC=0 PERFORMS THE STANDARD SPIN SUM
C THE OPTION IMC=1 PERFORMS THE CALCULATION FOR 'NMC' RANDOMLY
C CHOSEN HELICITY STATES WHICH IMPROVES THE
C SPEED BY A FACTOR 32/NMC
C     IMPLICIT NONE
      SAVE
...
C CHECK ON EITHER FIRST CALL OR CHANGE IN IMC
      IF(INIT.EQ.0.OR.IMC.NE.OLDIMC) THEN
        OLDIMC=IMC
        INIT=1
C REPRODUCE INPUT DATA
C       WRITE(6,*) ' ----------------------------------------'
...
C       WRITE(6,*) ' LEPTON AXIAL C.   = ',AL
        RMB=RMQ
C ADJUST STRONG COUPLING SO AS TO GIVE EFFECTIVELY ALPHA_S=1
        GSTR=4D0*DSQRT(DATAN(1D0))
C       WRITE(6,*) ' QCD COUPLING      = ',GSTR
C SEE WETHER GAUGE CHECKS ARE REQUIRED
        IF(CHKGL1.EQ.1) THEN
        ...
        ENDIF
C SEE WETHER HELICITY MONTE CARLO IS REQUIRED
        IF(IMC.EQ.0) THEN
          KLOW=1
          KUPP=32
          MULT=1D0
...
C INITIALIZE THE ARRAYS ANSS,DONS
      DO 130 J1=-1,1,2
        DO 120 J2=1,4
          DO 110 J3=-1,1,2
            DO 100 J4=1,4
...
C INITIALIZE THE ARRAYS ANSF,DONF
      DO 180 J1=-1,1,2
        DO 170 J2=1,4
          DO 160 J3=1,8
            DO 150 J4=-1,1,2
...
C EQUATE THE (0:4) INTERNAL MOMENTA TO THE (0:3) ARGUMENTS MOMENTA
      DO 190 K=0,3
        K1(K)=AK1(K)
        K2(K)=AK2(K)
        P1(K)=AP1(K)
...
C ASSIGN LABELS TO THE MOMENTA FOR RECOGNITION
C THE MOMENTA K1,K2,LEP1,LEP2 (AND R1,R2) CAN OCCUR AS THE MASSLESS
C MOMENTA IN ARGUMENTS NO.2 AND 6 IN ZF, AND NO.2 AND 4 IN RKZSF
C R1,R2 AND Q1,Q2 ARE SOME OF THESE, AND CAN ALSO OCCUR
C AS ARGUMENTS NO.2 AND 6 IN ZF AND NO.2 AND 4 IN RKZSF
        K1(4)=1D0
        K2(4)=2D0
        LEP1(4)=3D0
        LEP2(4)=4D0
C THE OTHER MOMENTA P1,P2 AND THE VARIOUS RR1,RR2 CAN OCCUR ONLY
C AS ARGUMENT NO.3 IN ZF
        P1(4)=1D0
        P2(4)=2D0
 
C THE TOTAL BOSON MOMENTUM
C NO NEED TO ASSIGN 4TH COMPONENT LABEL SINCE IT IS NOT USED
      DO 200 K=0,3
        QV(K)=LEP1(K)+LEP2(K)
  200 CONTINUE
 
 
C DEFINE THE AUXILIARY VECTORS: THE RESULT SHOULD BE THE SAME
C FOR EVERY NON-SINGULAR CHOICE OF THE AUXILIARY VECTORS
C SINGULAR CHOICES ARE R1=K1 OR R2=K2
C THESE ARE OBTAINED BY PUTTING CHKGL1=1 OR CHKGL2=1
 
C AUXILIARY VECTOR FOR GLUON 1
C NEED TO ASSIGN ALSO 4TH COMPONENT LABELS HERE!
      IF(CHKGL1.EQ.1) THEN
        DO 210 K=0,4
          R1(K)=K1(K)
  210   CONTINUE
...
C AUXILIARY VECTOR FOR GLUON 2
      IF(CHKGL2.EQ.1) THEN
        DO 230 K=0,4
          R2(K)=K2(K)
  230   CONTINUE
...
C AUXILIARY VECTOR FOR THE B QUARK
      DO 250 K=0,4
        Q1(K)=LEP1(K)
  250 CONTINUE
 
C AUXILIARY VECTOR FOR THE B_BAR QUARK
      DO 260 K=0,4
        Q2(K)=LEP2(K)
  260 CONTINUE
 
C INITIALIZE THE CROSS SECTION TO ZERO
      CROSS=0D0
 
C SINCE P2 CORRESPONDS TO AN ANTIFERMION WE HAVE TO
C CHANGE ITS SIGN MOMENTARILY: PUT THE OLD RESULT IN PP2(0:3)
C BU MAKE SURE TO KEEP THE LABEL POSITIVE!
      DO 270 K=0,3
        PP2(K)=P2(K)
        P2(K)=-P2(K)
  270 CONTINUE
 
C COMPUTE OVERALL FACTORS: FOR EVERY SLASHED POLARIZATION THERE
C APPEARS A FACTOR OF 2 IN ADDITION TO THE NORMALIZATION
C FOLLOWING FROM THE CHISHOLM IDENTITY
C IN PRINCIPLE THE OVERALL FACTORS ARE DIFFERENT FOR EACH DIFFERENT
C HELICITY COMPBINATION BUT IN THIS CASE WE ARE ONLY INTERESTED IN
C THEIR ABSOLUTE VALUE (NO TRANSVERSE GLUON POLARIZATION ETC.)
C SO WE CAN TAKE THIS OUT OF THE LOOP, EXCEPT FOR THE NONTRIVIAL
C HELICITY DEPENDENCE IN 'ZFACV'
 
C OVERALL FACTOR FOR THE BOSON CURRENT, WITH BREIT-WIGNER
      ZFACV=2./CMPLX(SNGL(RKDOT(QV,QV))-RMV**2,RMV*RGV)
 
C OVERALL FACTOR FOR GLUON 1
      IF(CHKGL1.EQ.1) THEN
        ZFAC1=(1.,0.)
      ELSE
C ORIGINAL FORM: ZFAC1=2D0*LG1/(DSQRT(2D0)*RKZPR(-LG1,K1,R1))
        ZFAC1=DSQRT(2D0)/RKZSF(1,K1,-1,R1)
      ENDIF
 
C OVERALL FACTOR FOR GLUON 2
      IF(CHKGL2.EQ.1) THEN
        ZFAC2=1D0
      ELSE
C ORIGINAL FORM: ZFAC2=2D0*LG2/(DSQRT(2D0)*RKZPR(-LG2,K2,R2))
        ZFAC2=DSQRT(2D0)/RKZSF(1,K2,-1,R2)
      ENDIF
 
C OVERALL FACTOR FOR QCD COUPLINGS
      ZFACS=GSTR**2
 
C OVERALL FACTOR FOR THE B QUARK
      ZFACB=1/DSQRT(2D0*RKDOT(P1,Q1))
 
C OVERALL FACTOR FOR THE B_BAR QUARK
      ZFACBB=1D0/DSQRT(2D0*RKDOT(PP2,Q2))
 
C FINAL OVERALL FACTOR
      ZFAC=ZFACV*ZFAC1*ZFAC2*ZFACS*ZFACB*ZFACBB
 
C DO A BIG LOOP OVER ALL HELICITIES OR A RANDOM CHOICE OF HELICITIES
C NB: FUNNY INDENTATION HERE!
C ALSO INITIALIZE COUNTERS FOR RKZSF AND ZF
...
C DETERMINE THE 'LEFT-' AND 'RIGHT-'HANDED COUPLINGS OF THE B TO THE Z
      VPA=VB+LV*AB
      VMA=VB-LV*AB
C AND THE LEPTON HELICITY FACTOR
      ZFACL=(VL-LV*AL)
 
C FIRST PART OF THE RESULT: THE ABELIAN TERMS
C COMPUTE THE NUMERATORS (ZN...) USING THE ZF FUNCTION
C AND THE DENOMINATORS (ZD...) THE STANDARD WAY
C THE INTERNAL FERMION MOMENTA ARE DIFFERENT IN EACH DIAGRAM
C AND ARE DENOTED BY RR1 AND RR2
C THE 4TH COMPONENT LABELS ARE NONTRIVIAL HERE: HAVING ALREADY
C P1(4)=1 AND P2(4)=2 WE ALSO DEFINE
C (P1-K1)(4)=3,
C (P1-K1-K2)(4)=(P1-K2-K1)(4)=4
C (P1-K2)(4)=5
C (P1-K1+QV)(4)=6
C (P1-K2+QV)(5)=7
C (P1+QV)(4)=8
C SO THAT IN THE VARIOUS DIAGRAMS WE HAVE
C IN ZN12V: RR1(4)=3, RR2(4)=4
C IN ZN21V: RR1(4)=5, RR2(4)=4
C IN ZN1V2: RR1(4)=3, RR2(4)=6
C IN ZN2V1: RR1(4)=5, RR2(4)=7
C IN ZNV12: RR1(4)=8, RR2(4)=6
C IN ZNV21: RR1(4)=8, RR2(4)=7
...
C SECOND PART OF THE RESULT: THE NONABELIAN PART.
C THIS IS MADE UP PARTLY FROM THE ABELIAN PART AND PARTLY FROM
C NEW PIECES
C THE ASSIGNMENT OF THE 4TH COMPONENT LABELS IS NOW UNNECESSARY
C FOR RR1 SINCE IT DOES NOT OCCUR IN ANY ZF HERE
...
C THE FOURTH COMBINATION CAN BE GOTTEN FROM
C THE FIRST THREE USING DIRAC ALGEBRA:
C EPS1*EPS2*EPVS+EPS2*EPS1*EPSV = 2(EPS1.EPS2)*EPSV ETC.
      ZCV21=ZC12V+ZC21V-ZCV12
 
      ZDIA8=(-ZNV12+ZNV21)/ZD22-(ZCV12-ZCV21)/(2D0*S)
 
C CONSTRUCT THE ABELIAN AND NONABELIAN PART
...
C (N**2-1)*(N**2-2)/(8*N) = 7/3 FOR THE ABELIAN PART
C N*(N**2-1)/8 = 3 FOR THE NONABELIAN PART
C AND ADD THE RESULT TO THE CROSS SECTION
      THIS1=7D0/3D0*ABS(ZABEL)**2+3D0*ABS(ZNABEL)**2
CC    WRITE(6,801)HELIX,LG1,LG2,LV,L1,L2,THIS1
CC801 FORMAT(' ',6I4,D30.20)
      CROSS=CROSS+THIS1
 
C END OF THE BIG LOOP OVER HELICITIES
  360 CONTINUE
 
C DO NOT FORGET TO PUT P2 BACK TO ITS ORIGINAL VALUE IN PP2!
      DO 370 K=0,3
        P2(K)=PP2(K)
  370 CONTINUE
 
C ADD AVERAGING FACTORS:
C 1/2 FOR EACH GLUON SPIN, 1/8 FOR EACH GLUON COLOUR
      CROSS=CROSS/256D0
 
C TAKE INTO ACCOUNT A POSSIBLE FACTOR FOR THE HELICITY SUM OPTION
C AND RETURN THE FINAL RESULT
