...
C...jet system according to the Lund string fragmentation model.
     ...
     &TJU(5),KFJH(2),NJS(2),KFJS(2),PJS(4,5),MSTU9T(8),PARU9T(8)
      ...
      dblefour(I,J)=dble(P(I,4))*dble(P(J,4))-
     > dble(P(I,1))*dble(P(J,1))-dble(P(I,2))*dble(P(J,2))-
     > dble(P(I,3))*dble(P(J,3))
...
C...Express pT with respect to new axes, if sensible.
     ...
     &  FOUR(IN(3*JT+3)+1,IN(3)))
        ...
        do 12021 k12 = 1, 5
          xmyv1(k12) = p(in(3*jt+3)+1,k12)
          xmyv2(k12) = p(in(3)+1,k12)
12021   continue

        MY_FOUR1 = dblefour(IN(3*JT+3), IN(3)+1)
        MY_FOUR2 = dblefour(IN(3*JT+3)+1, IN(3)+1)

        PYP=-(PX(3)*dblefour(IN(3*JT+3),IN(3)+1)+PY(3)*
     &  dblefour(IN(3*JT+3)+1,IN(3)+1))
...
C...Solve kinematics for final two hadrons, if possible.
  ...
  820 P(I,J)=P(N+NRS,J)-P(I-1,J)
      IF(P(I-1,4).LE.0..OR.P(I,4).LE.0.) GOTO 550
...
C...Document string system. Move up particles.
  ...
  850 V(I,J)=0.
      MSTU91=MSTU(90)
      DO 855 IZ=MSTU90+1,MSTU91
      MSTU9T(IZ)=MSTU(90+IZ)-NRS+1-NSAV+N
  855 PARU9T(IZ)=PARU(90+IZ)
      MSTU(90)=MSTU90
