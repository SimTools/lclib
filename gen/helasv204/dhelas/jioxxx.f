...
C
      ...
      VM2=VMASS**2
C
      IF (VMASS.EQ.0.D0) GOTO 50
C
      D=1.D0/DCMPLX( Q2-VM2 , DMAX1(DSIGN( VMASS*VWIDTH ,Q2),0.D0) )
C  For the running width, use below instead of the above D.
C      D=1.D0/DCMPLX( Q2-VM2 , DMAX1( VWIDTH*Q2/VMASS ,0.D0) )
C
      IF (G(2).EQ.0.D0) GOTO 10
C
     ...
     &    +G(2)*(-FO(1)*FI(4)+FO(2)*FI(3)))*DCMPLX(0.D0,1.D0)
...
C
  10  D=D*G(1)
      ...
      C2=( FO(3)*FI(2)-FO(4)*FI(1))*DCMPLX(0.D0,1.D0)
...
C
     ...
     &          +G(2)*(-FO(1)*FI(4)+FO(2)*FI(3)) )*DCMPLX(0.D0,DD)
...
C
      ...
      JIO(3) =  ( FO(3)*FI(2)-FO(4)*FI(1))*DCMPLX(0.D0,DD)
