...
C
      include 'jpidef.inc'
C      
      CALL LIB$GETJPI( JPI$_MODE,,,IRET )
C
      IF( IRET .EQ. JPI$K_INTERACTIVE ) THEN
        IFTSS = .TRUE.
      ELSE
        IFTSS = .FALSE.
      END IF
...
C
C BEGIN
      CALL LIB$SPAWN( COMAND(:LCMD),,,,,IRET1 )
      IRET2 = IRET1
      RETURN
      END
C
...
C
C BEGIN
      CALL STR$UPCASE( STRING,STRING(:LEN) )
      RETURN
      END
C
C
...
100   CONTINUE
      ...
      ISTAT = LIB$FIND_FILE( INPUT,OUTPUT(NO+1),ICONT )
      If( .NOT. BJTEST( ISTAT,0 ) ) GOTO 900
      NO = NO + 1
      GOTO 100
900   CONTINUE
